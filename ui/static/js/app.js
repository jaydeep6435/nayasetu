/**
 * NayaSetu — Frontend Application Logic (Redesigned)
 *
 * Architecture:
 *   1.  State Management
 *   2.  DOM References
...
const STORAGE_KEY = 'nayasetu_sessions';
...
async function handleQuery(question) {
  ...
  try {
    ...
    console.error('[NayaSetu] Query error:', error);
  }
...
function bindEvents() {
  ...
  $('#about-btn').addEventListener('click', () => {
    const text = 'NayaSetu is an AI-powered legal rights navigator for Indian citizens. Built with FastAPI, LangChain, ChromaDB, SentenceTransformers, and Groq Llama 3.1.';
    alert(text);
  });
...
function init() {
  ...
  console.log('[NayaSetu] App initialized. Backend API: /api/query, /api/health, /api/categories');
}
